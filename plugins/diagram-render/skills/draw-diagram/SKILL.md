...
| 観点               | フロー図 (フローチャート系)                            | アーキテクチャ図 (構造系)                                                                                          |
| ------------------ | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| 表すもの           | **時間的順序 / 処理の流れ / 判断分岐**                 | **静的な構造 / コンポーネント配置 / 接続関係**                                                                     |
| ノードの粒度       | 「処理」「判断」「入出力」など動作                     | 「サービス」「データストア」「外部システム」など実体                                                               |
| 矢印の意味         | 「次に進む」(時間軸)                                   | 「通信する」「依存する」(構造的関係)                                                                               |
| 同じノードの再登場 | 1 回 (進行表現なのでループは明示的に戻り矢印で書く)    | 1 回 (実体は1つ)                                                                                                   |
| 推奨向き           | `TD` (上→下)。**時間軸が縦に流れる**ので意味がある     | **どちらでも良い**。実体の繋がりに時間軸はなく向きは構造的に意味を持たない。レイアウトの都合 (横長 vs 縦長) で選ぶ |
| Mermaid 図種       | `flowchart TD` / `sequenceDiagram` / `stateDiagram-v2` | `graph LR` + `subgraph`                                                                                            |
| 典型シェイプ       | `([Start])` `{判断}` `[/入力/]`                        | `[Service]` `[(DB)]` `[[Queue]]`                                                                                   |
| 凡例               | "OK"/"NG" など分岐条件、動詞 (送信する、保存する)      | プロトコル名・呼び出し方 (`POST /api`, `gRPC`, `pub/sub`)                                                          |
| 例                 | リクエスト受信 → バリデーション → DB 保存 → 応答       | Web → API Gateway → Lambda → RDS / S3                                                                              |
...
| 用途                    | Mermaid 図種                          | 使うべき場面                                                         |
| ----------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| アーキテクチャ図        | `graph LR` / `flowchart` + `subgraph` | サービス・レイヤー・コンポーネントの構造を俯瞰したい                 |
| システム構成図          | `graph LR` + `subgraph`               | クラウド/オンプレ含む実体配置 (VPC, Region, K8s namespace 等)        |
| フローチャート          | `flowchart TD`                        | 処理手順・分岐・判断ロジックを表現                                   |
| 依存関係図              | `graph LR`                            | モジュール/パッケージ/サービス間の依存方向を示す                     |
| シーケンス図            | `sequenceDiagram`                     | 時系列でのコンポーネント間メッセージのやり取り (API呼び出し順序など) |
| ER図                    | `erDiagram`                           | DB テーブル・エンティティのリレーション                              |
| 状態遷移図              | `stateDiagram-v2`                     | ステートマシン (注文ステータス, トークンライフサイクル等)            |
| クラス図                | `classDiagram`                        | OOP の継承/合成関係                                                  |
| ガントチャート          | `gantt`                               | スケジュール・マイルストーン                                         |
| パイチャート / XY chart | `pie` / `xychart-beta`                | 比率や数値分布                                                       |
