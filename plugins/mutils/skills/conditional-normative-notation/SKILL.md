...
| キーワード   | 意味                                     | 使いどころ                             |
| ------------ | ---------------------------------------- | -------------------------------------- |
| `MUST`       | 絶対要件。違反は許されない               | 破壊的操作の確認、不可逆操作の前提条件 |
| `MUST NOT`   | 絶対禁止                                 | やると壊れる・取り返しがつかない行為   |
| `SHOULD`     | 原則やるべき。逸脱には正当な理由が要る   | 守ってほしい既定。例外を許容したい規範 |
| `SHOULD NOT` | 原則避けるべき。逸脱には正当な理由が要る | 推奨されないが状況により許容           |
| `MAY`        | 任意。やってもやらなくてもよい           | 判断を読み手に委ねたい                 |
...
- **条件 (`IF`)** — 読み手が**真偽を一意に判定できる**こと。「重要な変更のとき」は不可（重要かは主観）。「破壊的変更を実行しようとしている」は可（実行内容から判定できる）。
- **キーワード** — 後述の選び方に従い、強制力に見合うものを1つ選ぶ。
- **行動 (`THEN`)** — その項目単独を読んで**実施内容が一意に決まる**粒度。「適切に対応する」は不可。「実行前にユーザーに確認する」は可。
