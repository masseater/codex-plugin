...
MUST NOT: report a finding without concrete `evidence` (speculation is not a finding — drop it instead).

## Workflow

### Step 1: Establish Scope

- IF: scope is ambiguous; THEN MUST: ask via `AskUserQuestion` the following
- IF: scope is not ambiguous; THEN MUST: assume the whole repo without asking
- IF: the user already specified scope in their prompt; THEN MUST NOT: ask the scope question

Questions to ask when scope is ambiguous:
...
MUST: run these in parallel and record raw output for evidence. SHOULD: prefer Bash in parallel tool calls:
...
- IF: the repo is large; THEN SHOULD: delegate broad searches to an `Explore` subagent with `thoroughness: medium`
- IF: spawning a subagent; THEN MUST: pass `model: sonnet` (or `opus`) — AGENTS.md forbids haiku subagents, and Explore's default may resolve to haiku in some harness configurations
...
MUST: also emit the same data as a fenced JSON block so downstream skills (`github-issue-db:search-similar-issues`, `github-issue-db:report-repo-issues`) can parse it without re-scraping Markdown:
...
- MUST: cite concrete evidence only; MUST NOT use "possibly", "might be", "perhaps"
- SHOULD: prefer fewer, well-evidenced findings over many weak ones
- MUST: respect `.claude/rules/ai-generated/gotchas.md` when interpreting signals (e.g. JSONL parsing rule)
- MUST: communicate with the user in 日本語; produce finding content in English for downstream reuse
- MUST NOT: call `gh`/Octokit here — this skill is local-only (defer issue creation to `github-workflow:create-issue` or `github-issue-db:report-repo-issues`)
