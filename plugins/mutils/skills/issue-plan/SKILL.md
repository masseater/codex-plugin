...
IF: 新規実装を検討する; THEN MUST: 先に既存実装を確認する
...
MUST: TodoWriteツールを使用して、実装タスクをTodoListとして登録する。
タスクは以下の形式で:
...
- MUST NOT: `any` 型を使用する（代わりに具体的な型 / ジェネリクス / 型ガードで表現する）
- MUST NOT: 場当たり的な対応をする（「一時的」「とりあえず」等の表現を使わず、代わりに根本的な解決を設計する）
- MUST: 根本的な解決を目指す
- MUST: 既存の実装パターンを確認する
- SHOULD: プライドを持って設計を行う
