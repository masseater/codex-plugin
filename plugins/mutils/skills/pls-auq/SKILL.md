...
- MUST: これから質問する内容を全て出力してから AskUserQuestion する
- MUST NOT: 確認事項をまとめて1問にする（代わりに個別の質問として聞く）
- IF: 確認事項が複数ある; THEN MUST: AskUserQuestion の制約内で一つにまとめず最大限同時に質問する
- IF: 回答に応じてファイルを編集する; THEN MUST: 「どのファイル」の「どの部分」を「どのように」編集するかを質問の description に含める
...
IF: 質問が5つ以上ある; THEN MUST: 最初の4つを聞いて回答を待ち、残りを次に聞く。
...
MUST: AskUserQuestion の最大数は4つ（2026-01-22 現在）なので、4つごとにまとめる。
